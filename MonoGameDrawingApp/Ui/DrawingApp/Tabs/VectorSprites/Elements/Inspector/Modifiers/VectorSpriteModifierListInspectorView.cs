...
namespace MonoGameDrawingApp.Ui.DrawingApp.Tabs.VectorSprites.Elements.Inspector.Modifiers
{
    public class VectorSpriteModifierListInspectorView : IUiElement
    {
        ...
        private IVectorSpriteItemModifier[] _modifiers;
        private IUiElement[] _modifierElements;
        ...
        public void Reload()
        {
            ...
            IEnumerable<IVectorSpriteItemModifier> itemModifiers = VectorSpriteTabView.Selected.Modifiers;

            if (_modifiers?.Length != itemModifiers.Count())
            {
                _modifiers = new IVectorSpriteItemModifier[itemModifiers.Count()];
                _modifierElements = new IUiElement[itemModifiers.Count()];
            }

            int i = 0;
            foreach (IVectorSpriteItemModifier modifier in itemModifiers)
            {
                if (modifier != _modifiers[i])
                {
                    _modifiers[i] = modifier;
                    _modifierElements[i] = new VectorSpriteModifierInspectorView(Environment, modifier, VectorSpriteTabView);
                }
                ++i;
            }
